...
/**
 * Given a hand notation and parsed raise/call/fold maps, return action frequencies.
 * With combo frequencies, raise + call + fold = rangeFreq (not 1.0).
 * The remaining (1 - rangeFreq) is "not in range".
 *
 ...
 *   If null, all hands are treated as "in range" based on action data.
 */
export function getCellData(handNotation, parsedRaise, parsedCall, parsedRaise2 = {}, parsedFold = {}, inRangeSet = null) {
  ...
  const foldFreq   = parsedFold[handNotation]   ?? 0

  const combined = raiseFreq + raise2Freq + callFreq + foldFreq

  // Check if hand is "in range" (has any action data)
  const isInRange = inRangeSet ? inRangeSet.has(handNotation) : (combined > 0.001)

  // "Not in range" frequency is the portion not covered by any action
  const notInRangeFreq = Math.max(0, 1 - combined)

  return { raiseFreq, raise2Freq, callFreq, foldFreq, notInRangeFreq, isInRange }
}
